  ...
  return [
    {
      ...
      content: [
        profile.tagline,
        profile.bio,
        profile.birthDate ? `Date of birth: ${profile.birthDate}` : "",
        profile.hobbies && profile.hobbies.length > 0 ? `Hobbies and interests: ${profile.hobbies.join(", ")}` : "",
        `Location: ${profile.location}`,
        `Roles: ${profile.roles.join(", ")}`,
        `Available for work: ${profile.availableForWork ? "yes" : "no"}`,
        `Stats: ${profile.stats.map((s) => `${s.value}${s.suffix} ${s.label}`).join(", ")}`,
      ].filter(Boolean).join("\n"),
      searchableTerms: "about bio profile summary overview introduction background who himself herself person developer engineer age born birthday birth date dob hobbies hobby interests free time politics teach guide mysteries tools ai software",
    },
    {
      id: "contact",
      category: "contact",
    ...
    ...portfolio.projects.map((project) => ({
      ...
      content: [
        ...
        !project.githubUrl && !project.github && project.projectType?.toLowerCase().includes("private") ? "Repository availability: private or hidden repository; GitHub link available to recruiters on request until publication." : "",
