  ...
  return (
    <AnimatePresence>
      {visible && (
        ...
        >
          {/* Isolated background - no global style interference */}
          {/* base gradients with inline blur (not relying on global glow layers) */}
          <div 
          ...
          {/* isolated grid - NOT using global .grid-bg */}
          <div 
          ...
          {/* scan line - isolated animation, not using global class */}
          <motion.div
            animate={{
              y: ["-5%", "105%"],
            }}
          ...
          <div className="relative z-10 flex w-full max-w-[440px] flex-col items-center px-6">
            ...
            >
              {/* outer glow */}
              <div className="absolute inset-0 rounded-[28px] bg-primary/30 blur-2xl" />

              {/* box - fully isolated, NO .glass class */}
              <div 
            ...
            >
              ...
              {/* lines */}
              <div className="h-[108px] space-y-1.5 px-4 py-4 font-mono text-[0.72rem] leading-5">
                {BOOT_LINES.slice(0, lineIndex + 1).map(
                  (line, i) => (
                    ...
                    >
                      ...
                      {i === lineIndex && (
                        <span className="ml-1 inline-block h-3 w-[7px] animate-pulse bg-cyan-300/90 align-middle" />
                      )}
                    </motion.div>
                  )
                )}
          ...
          {/* corners */}
          <div className="absolute left-5 top-5 font-mono text-[0.62rem] tracking-[0.14em] text-primary/30">
            [ SYSTEM_READY ]
          </div>

          <div className="absolute right-5 top-5 font-mono text-[0.62rem] tracking-[0.14em] text-primary/30">
            v3.1.0
          </div>

          <div className="absolute bottom-5 left-5 font-mono text-[0.62rem] tracking-[0.14em] text-primary/20">
            SECURE_CONN : ACTIVE
          </div>

          <div className="absolute bottom-5 right-5 font-mono text-[0.62rem] tracking-[0.14em] text-primary/20">
            NODE_STATUS : ONLINE
          </div>
        </motion.div>
      )}
